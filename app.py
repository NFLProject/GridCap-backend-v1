...
app = FastAPI(title="GridCap API (v1)", version="1.0")

ALLOWED_ORIGINS = [
    os.getenv("FRONTEND_ORIGIN", "https://your-vercel-app.vercel.app"),
    "http://localhost:3000",
]
app.add_middleware(
    CORSMiddleware,
...
class LeagueCreateIn(BaseModel):
    name: str = "GridCap League"; team_name: str
class LeagueJoinIn(BaseModel):
    league_id: int; team_name: str
...
class GWCreateIn(BaseModel):
    id: int; name: Optional[str] = None; deadline_at: Optional[datetime] = None

# -------------------- Auth routes --------------------
@app.post("/auth/register")
def auth_register(inp: RegisterIn, db: Session = Depends(get_db)):
    if db.query(User).filter(User.email == inp.email.strip().lower()).first():
        raise HTTPException(400, "Email already registered")
    u = User(name=inp.name.strip(), email=inp.email.strip().lower(), password_hash=bcrypt.hash(inp.password))
...
def auth_login(inp: LoginIn, db: Session = Depends(get_db)):
    u = db.query(User).filter(User.email == inp.email.strip().lower()).one_or_none()
    if not u or not bcrypt.verify(inp.password, u.password_hash):
...
def league_create(inp: LeagueCreateIn, current: User = Depends(get_current_user), db: Session = Depends(get_db)):
    league = League(name=inp.name, owner_id=current.id); db.add(league); db.commit(); db.refresh(league)
    entry = Entry(league_id=league.id, user_id=current.id, team_name=inp.team_name); db.add(entry); db.commit(); db.refresh(entry)
...
async def sync_players(db: Session = Depends(get_db)):
    ...
    for sid, p in payload.items():
        ...
        if obj:
            obj.name = name; obj.team = team; obj.pos = pos_app; obj.price_m = price_m; updated += 1
        else:
...
def set_lineup(inp: LineupSetIn, current: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    if obj:
        obj.starters_json = starters_json; obj.captain_id = inp.captain_id; obj.vice_captain_id = inp.vice_captain_id; obj.chip = inp.chip
    else:
...
def standings(league_id: int, current: User = Depends(get_current_user), db: Session = Depends(get_d
